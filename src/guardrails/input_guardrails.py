...
def detect_injection(user_input: str) -> bool:
    ...
    INJECTION_PATTERNS = [
        ...
        r"(reveal|show|print|display)\s+(your\s+)?(system\s+prompt|instructions?)",
        r"\byou are now\b",
        r"\bpretend you are\b",
        r"\bact as (a|an)?\s*unrestricted\b",
        r"\boverride\b.*\b(system|safety|policy)\b",
        r"\bforget\b.*\binstructions?\b",
        r"\bb[ỏo]\s+qua\b.*\bh[ướ]ng d[ẫa]n\b",
    ]
    ...
    async def on_user_message_callback(
        ...
        *,
        ...
        if topic_filter(text):
            self.blocked_count += 1
            return self._block_response(
                "I can only help with safe banking-related questions for VinBank."
            )
